...
class Logic3Test {
    ...
    def test1 {
        ...
        val a = Name(model.getItem("A").setType("tristate"))
        val b = Name(model.getItem("B").setType("tristate"))
...
//        val f = And(a, b)
    ...
    def checkF(f: Expr) = {
        ...
        val assignments: List[Map[Item, Char]] = getAllAssignments(vars)
    ...
    def getAllAssignments(items: Iterable[Item]): List[Map[Item, Char]] = if (items.isEmpty) List(Map())
    else {
